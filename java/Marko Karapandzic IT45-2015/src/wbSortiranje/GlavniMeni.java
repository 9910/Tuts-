...
public class GlavniMeni extends JFrame {
	...
	public GlavniMeni() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(dlgD.isDodaj()){
					Kvadrat kvad = new Kvadrat(
							dlgD.getGoreLevo(), 
							dlgD.getDuzinaStranice(), 
							dlgD.getBojaIvice(), 
							dlgD.getBojaUnutrasnjosti()
					);
					kv.addElement(kvad);
				}
			}
		...
		btnSortiraj.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				for (int i = 0 ; i < ( kv.getSize() - 1 ); i++){
				    for (int d = 0 ; d < kv.getSize() - i - 1; d++){
				      if (kv.getElementAt(d).povrsina() > kv.getElementAt(d + 1).povrsina()){
				        Kvadrat pomicni = kv.getElementAt(d);
				        kv.setElementAt(kv.getElementAt(d + 1), d);
				        kv.setElementAt(pomicni, d + 1);
				      }
				    }
				 }
			}
